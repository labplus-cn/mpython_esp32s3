    ...
    wl_handle_t* wl_handle)
{
    ...
    result = ffs_diskio_register_wl_partition(pdrv, *wl_handle);
    if (result != ESP_OK) {
        ESP_LOGE(TAG, "ffs_diskio_register_wl_partition failed pdrv=%i, error - 0x(%x)", pdrv, result);
        goto fail;
    }
    FATFS *fs;
    ...
    if (fresult != FR_OK) {
        ...
        workbuf = ffs_memalloc(workbuf_size);
        if (workbuf == NULL) {
        ...
        fresult = fs_mkfs(drv, &opt, workbuf, workbuf_size);
        if (fresult != FR_OK) {
            result = ESP_FAIL;
            ESP_LOGE(TAG, "fs_mkfs failed (%d)", fresult);
            goto fail;
        }
        free(workbuf);
        workbuf = NULL;
        ESP_LOGI(TAG, "Mounting again");
        fresult = fs_mount(fs, drv, 0);
        if (fresult != FR_OK) {
            result = ESP_FAIL;
            ESP_LOGE(TAG, "fs_mount failed after formatting (%d)", fresult);
            goto fail;
        }
    }
    return ESP_OK;
...
{
    BYTE pdrv = ffs_diskio_get_pdrv_wl(wl_handle);
    if (pdrv == 0xff) {
    ...
    fs_mount(0, drv, 0);
    ffs_diskio_unregister(pdrv);
    ffs_diskio_clear_pdrv_wl(wl_handle);
    ...
    const esp_vfs_fat_mount_config_t* mount_config)
{
    ...
    result = ffs_diskio_register_raw_partition(pdrv, data_partition);
    if (result != ESP_OK) {
