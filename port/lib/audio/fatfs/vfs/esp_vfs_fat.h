 ...
 * This function also allocates FATFS structure which should be used for fs_mount
 * call.
 *
 ...
 * @param[out] out_fs  pointer to FATFS structure which can be used for FATFS fs_mount call is returned via this argument.
 * @return
 *      - ESP_OK on success
 ...
 *       this call. Make sure to call fs_mount function to unmount it before
...
typedef struct {
    ...
    /**
     * If format_ifs_mount_failed is set, and mount fails, format the card
    ...
    /**
     * Enables real ffs_disk_status function implementation for SD cards
     * (ffs_sdmmc_status). Possibly slows down IO performance.
     *
