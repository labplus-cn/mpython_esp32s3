...
DSTATUS disk_initialize_1 (BYTE pdrv)
{
    return s_impls[pdrv]->init(pdrv);
}
DSTATUS disk_status_1 (BYTE pdrv)
{
    return s_impls[pdrv]->status(pdrv);
}
DRESULT disk_read_1 (BYTE pdrv, BYTE* buff, DWORD sector, UINT count)
{
    return s_impls[pdrv]->read(pdrv, buff, sector, count);
}
DRESULT disk_write_1 (BYTE pdrv, const BYTE* buff, DWORD sector, UINT count)
{
