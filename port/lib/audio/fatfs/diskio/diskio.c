...
void ffs_diskio_register(BYTE pdrv, const ffs_diskio_impl_t* discio_impl)
{
    ...
    if (s_impls[pdrv]) {
        ffs_diskio_impl_t* im = s_impls[pdrv];
        s_impls[pdrv] = NULL;
        free(im);
    }
    ...
    ffs_diskio_impl_t * impl = (ffs_diskio_impl_t *)malloc(sizeof(ffs_diskio_impl_t));
    assert(impl != NULL);
    memcpy(impl, discio_impl, sizeof(ffs_diskio_impl_t));
    s_impls[pdrv] = impl;
}

DSTATUS ffs_disk_initialize (BYTE pdrv)
{
    return s_impls[pdrv]->init(pdrv);
}
DSTATUS ffs_disk_status (BYTE pdrv)
{
    return s_impls[pdrv]->status(pdrv);
}
DRESULT ffs_disk_read (BYTE pdrv, BYTE* buff, LBA_t sector, UINT count)
{
    return s_impls[pdrv]->read(pdrv, buff, sector, count);
}
DRESULT ffs_disk_write (BYTE pdrv, const BYTE* buff, LBA_t sector, UINT count)
{
    return s_impls[pdrv]->write(pdrv, buff, sector, count);
}
DRESULT ffs_disk_ioctl (BYTE pdrv, BYTE cmd, void* buff)
{
