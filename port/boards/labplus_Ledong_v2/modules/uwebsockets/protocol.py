    ...
    def read_frame(self, max_size=None):
        ...
        try:
            if self.file_path == '':
                ...
                while True:
                    if length >= read_len:
                        data = self.sock.read(read_len).decode('utf-8')
                        index = data.find("\"audio\":\"") 
                        if (index > -1):
                            index +=9
                            data = data[index:]
                            data += self.sock.read(index).decode('utf-8')
                            length -= index
                            # print(data[0:30])
                        try:
                        ...
                        if length > 0:
                            data = self.sock.read(length).decode('utf-8')
                            index = data.find("\"audio\":\"") 
                            if (index > -1):
                                index +=9
                                data = data[index:]
