...
esp_err_t wav_head_parser(const uint8_t *inData, wav_info_t *info)
{
    ...
    info->bits_per_sample = inData[34];                   // bit length 16bit,24bit,32bit
    info->dataShift = 12 + 8 + inData[16] + 8;             // Data shift.
    info->fmtSubchunckSize = inData[16];
    info->fileChunkID = (((uint32_t)inData[3]) << 24) + (((uint32_t)inData[2]) << 16) + (((uint32_t)inData[1]) << 8) + inData[0];
    info->fileFormat = (((uint32_t)inData[11]) << 24) + (((uint32_t)inData[10]) << 16) + (((uint32_t)inData[9]) << 8) + inData[8];
    info->dataSize = (((uint32_t)inData[43]) << 24) + (((uint32_t)inData[42]) << 16) + (((uint32_t)inData[41]) << 8) + inData[40];
