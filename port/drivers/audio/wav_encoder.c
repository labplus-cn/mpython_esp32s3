...
static const char *TAG = "wav_recoder";
...
wav_decoder_t *wav_file_open(const char *path_in)
{
	...
	wr->lfs2_file = vfs_lfs2_file_open(path_in, LFS2_O_WRONLY);
	if(!wr->lfs2_file){
	...
	if (!wav_header){
		ESP_LOGE(TAG, "wav header calloc failt.");
	}
	wav_head_init(wav_header, renderer->sample_rate, renderer->bit_depth, renderer->i2s_channal_nums, data_len);
	lfs2_file_write(&wr->lfs2_file->vfs->lfs, &wr->lfs2_file->file, wav_header, sizeof(wav_header_t));
	free(wav_header);
